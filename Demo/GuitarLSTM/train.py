...
def main(args):
    ...
    name = args.name
    if not os.path.exists('Demo/Model/'+name):
        os.makedirs('Demo/Model/'+name)
    else:
    ...
    clear_session()
    model = Sequential()
    model.add(InputLayer(input_shape=(1, 1), name="Audio_In", dtype="float32"))
    ...
    if args.split_data > 1:
        ...
        model.save('Demo/Model/'+name+'/'+name+'.h5')
    ...
    save_wav('Demo/Model/'+name+'/y_pred.wav', prediction)
    save_wav('Demo/Model/'+name+'/x_test.wav', x_last_part)
    save_wav('Demo/Model/'+name+'/y_test.wav', y_test)

    # Add additional data to the saved model (like input_size)
    filename = 'Demo/Model/'+name+'/'+name+'.h5'
    ...
    if args.create_plots == 1:
        ...
        plot.analyze_pred_vs_actual({   'output_wav':'Demo/Model/'+name+'/y_test.wav',
                                            'pred_wav':'Demo/Model/'+name+'/y_pred.wav', 
                                            'input_wav':'Demo/Model/'+name+'/x_test.wav',
                                            'model_name':name,
                                            'show_plots':1,
                                            'path':'Demo/Model/'+name
                                        })
